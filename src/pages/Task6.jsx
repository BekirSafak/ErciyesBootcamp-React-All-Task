...
function Task6() {
    ...
    return (
        <div className='w-screen'>
            ...
            <div className='text-center font-bold text-lg mt-4'>Total: {products.length}</div>
            <div className='flex mx-auto justify-center w-full text-center'>
                <table className='w-4/5 mt-4'>
                    ...
                    <tbody className='p-4'>
                        {products.map((res) => (
                            <tr key={res.id}>
                                <td className='text-green-600 font-bold hover:text-green-400 duration-300'><Link to={`/task6/${res.id}`}>{res.id}</Link></td>
                                <td>{res.name}</td>
                                <td>{res.unitPrice}</td>
